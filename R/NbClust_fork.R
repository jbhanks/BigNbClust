NbC <-function(data = NULL, diss=NULL, distance ="euclidean", min.nc=2, max.nc=15, method =NULL, index = "all", alphaBeale = 0.1)
  ...
  #                                              Distances                                                               #
  #                                                                                                                      #
  # Can be left out because the Rfast version of hclust starts with the data matrix and does the distance calculation
  # Just do the clustering directly
  #&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&#
...
if(!is.null(diss))
{
  if((distanceM==1)||(distanceM==2)|| (distanceM==3)|| (distanceM==4)|| (distanceM==5)|| (distanceM==6))
    ...
    if (method == 1) 
    {
        hc<-fastcluster::hclust(as.dist(md),method = "ward.D2")      
    }
    if (method == 2) 
    {
        hc<-fastcluster::hclust(as.dist(md),method = "single")		
	  }
    if (method == 3)
     {
        hc<-fastcluster::hclust(as.dist(md),method = "complete")		
     }
	   
    if (method == 4) 
    {
        hc<-fastcluster::hclust(as.dist(md),method = "average")
    }
	  
    if (method == 5) 
    {
        hc<-fastcluster::hclust(as.dist(md),method = "mcquitty")
		
   	}
    if (method == 6) 
    {
        hc<-fastcluster::hclust(as.dist(md),method = "median")
			
	  }
    if (method == 7) 
    {
        hc<-fastcluster::hclust(as.dist(md),method = "centroid")
		 
	  }
    if (method == 9) 
    {
      hc<-fastcluster::hclust(as.dist(md),method = "ward.D")
    ...
    GAP <- function(X, cl, referenceDistribution, B, method, d, centrotypes) 
      {
        ...
        WkB <- matrix(0, 1, B)
        for (bb in (1:B)) {
          print(paste("WkB iteration is", as.character(bb)))
            ...
            if (bb == 1) {
                ...
                { set.seed(1)
                  ...
                  pp2 <- cutree(fastcluster::hclust(as.dist(Dist(Xnew)), method = method), 
                    ClassNr)
